...
df = pd.read_csv('D:/ticket/preprocessing_data.csv')

i = list(df.columns.values)
i.pop(i.index('total_amount'))
df0 = df[i+['total_amount']]
...
def gradientboosting():
	regressor = GradientBoostingRegressor(loss='squared_error',n_estimators=500, min_samples_split=15).fit(X_train, y_train)
	regressor.fit(X_train, y_train)
	y_predictions = regressor.predict(X_test)
